...
exports.getEnrollmentByCourseIdAndLearnerId = async (req, res) => {
    try {
        ...
        const enrollment = await Enrollment.findOne({ course: courseId, learnerId: learnerId });
...
exports.getEnrollmentByCourse = async (req, res) => {
    try {
        ...
        const enrollment = await Enrollment.find({ course: courseId });
        const learnerIds = enrollment.map(enrollment => enrollment.learnerId);

        const students = await Learner.find({ _id: { $in: learnerIds } });
